...
- stable external Android keystore and its four `RUSSICAPTOR_RELEASE_*` environment variables;
...
1. Update `applicationVersion`, monotonically increasing `versionCode`, filename, and required migrations in `release/field-release.json`; keep `app.json` aligned.
...
4. Distribute the APK together with `dist/field-release/release-manifest.json`. Only a manifest with `distributable: true` is a field release.

## Fresh install

Enable installation from the approved package source, install the APK, launch RussiCaptor, sign in with a provisioned production operator, and confirm the Supabase project reference shown on the login screen. Disable the temporary installation permission afterward.

...
export function InstructorEvaluationPanel({ source, readOnly = false }: { source: ExerciseEvaluationResult; readOnly?: boolean }) {
  ...
  const [editing, setEditing] = useState(false); const [pending, setPending] = useState(false); const [message, setMessage] = useState("Õiguste ja hinnangu laadimine…");
  useEffect(() => {
    let active = true;
  ...
  return <View style={styles.card}><View style={styles.header}><View style={styles.headerCopy}><Text style={styles.title}>Instruktori hinnang</Text><Text style={styles.subtitle}>Human-authored interpretation · separate from machine results</Text></View>{!readOnly && canWrite && !editing && <Pressable style={styles.edit} onPress={() => setEditing(true)}><Text style={styles.editText}>Muuda</Text></Pressable>}</View>
    ...
    {source.dimensions.map(dimension => { const dimensionValue = dimensions.get(dimension.dimensionId); return <View key={dimension.dimensionId} style={styles.dimension}><Text style={styles.dimensionTitle}>{dimension.title}</Text><JudgementControl value={dimensionValue?.judgement} disabled={!editing || readOnly} onChange={judgement => setDimension({ dimensionId: dimension.dimensionId, judgement, ...(dimensionValue?.comment ? { comment: dimensionValue.comment } : {}) })} />
      {(editing || dimensionValue?.comment) && <TextInput editable={editing && !readOnly} value={dimensionValue?.comment ?? ""} onChangeText={comment => setDimension({ dimensionId: dimension.dimensionId, judgement: dimensionValue?.judgement ?? "NOT_ASSESSED", ...(comment ? { comment } : {}) })} placeholder="Mõõtme kommentaar" multiline style={styles.input} />}
      {dimension.expectations.map((item, index) => { const key = `${dimension.dimensionId}:${item.expectationId}:${item.subjectId ?? ""}`; const value = expectations.get(key); return <View key={`${key}:${index}`} style={styles.expectation}><Text style={styles.machine}>{item.classification} · {item.status.replaceAll("_", " ")}</Text><Text style={styles.expectationId}>{item.expectationId}{item.patientId ? ` · ${item.patientId}` : ""}</Text><JudgementControl value={value?.judgement} disabled={!editing || readOnly} onChange={judgement => setExpectation({ dimensionId: dimension.dimensionId, expectationId: item.expectationId, ...(item.subjectId ? { subjectId: item.subjectId } : {}), judgement, ...(value?.comment ? { comment: value.comment } : {}) })} />{(editing || value?.comment) && <TextInput editable={editing && !readOnly} value={value?.comment ?? ""} onChangeText={comment => setExpectation({ dimensionId: dimension.dimensionId, expectationId: item.expectationId, ...(item.subjectId ? { subjectId: item.subjectId } : {}), judgement: value?.judgement ?? "NOT_ASSESSED", ...(comment ? { comment } : {}) })} placeholder="Ootuse kommentaar" multiline style={styles.input} />}</View>; })}</View>; })}
    {(editing || draft.overallComment) && <TextInput editable={editing && !readOnly} value={draft.overallComment ?? ""} onChangeText={overallComment => setDraft(current => ({ ...current, ...(overallComment ? { overallComment } : { overallComment: undefined }) }))} placeholder="Üldkommentaar" multiline style={styles.input} />}
    ...
    {view && <Text style={styles.meta}>Auditeeritavad versioonid: {view.history.map(item => item.revision).join(", ")}</Text>}
    {!!message && <Text style={message.includes("Saved") ? styles.success : styles.message}>{message}</Text>}
    {editing && !readOnly && <View style={styles.actions}><Pressable disabled={pending} style={styles.cancel} onPress={() => { setDraft(view?.evaluation ?? empty); setEditing(false); }}><Text>Tühista</Text></Pressable><Pressable disabled={pending} style={styles.save} onPress={() => void save()}><Text style={styles.saveText}>{pending ? "Salvestamine…" : "Salvesta versioon"}</Text></Pressable></View>}
    <Text style={styles.boundary}>Skoori, hinnet, läbitud/mitteläbitud otsust, pädevusotsust ega tehisaru hinnangut ei anta.</Text>
  </View>;
}

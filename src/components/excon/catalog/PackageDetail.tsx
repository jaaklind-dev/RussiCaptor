...
export function PackageDetail({ entry, active, activationMessage, activationError, onActivate }: Readonly<{ entry?: ExerciseCatalogEntry; active: boolean; activationMessage?: string; activationError?: string; onActivate: () => void }>) {
  ...
  return <View testID="catalog-package-detail" style={styles.card}>
    ...
    {activationMessage && <Text testID="catalog-activation-success" style={styles.success}>{activationMessage}</Text>}
    {activationError && <Text testID="catalog-activation-error" style={styles.error}>{activationError}</Text>}
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: "#005bbb", borderRadius: 10, padding: 13, alignItems: "center", marginTop: 18 }, disabled: { backgroundColor: "#8993a4" }, buttonText: { color: "#fff", fontWeight: "900" }, success: { color: "#1b5e20", fontWeight: "800", marginTop: 8, textAlign: "center" }, error: { color: "#9b1c1c", fontWeight: "800", marginTop: 8, textAlign: "center" }, note: { color: "#6b778c", fontSize: 11, marginTop: 8, textAlign: "center" },
});

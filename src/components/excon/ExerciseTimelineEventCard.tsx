...
export const ExerciseTimelineEventCard = memo(function ExerciseTimelineEventCard({ event, onPress }: { event: ExerciseTimelineEvent; onPress: (id: string) => void }) {
  return <Pressable style={styles.card} onPress={() => onPress(event.id)}>
    <View style={styles.header}><Text style={styles.time}>{formatSimulationTime(event.simulationTimeSec)}</Text><Text style={styles.category}>{icons[event.category]} {event.category}</Text><View style={[styles.severity, styles[event.severity.toLowerCase() as "info" | "warning" | "error"]]} /></View>
    <Text style={styles.title}>{event.title}</Text>
    {event.patientId && <Text style={styles.meta}>Patient: {event.patientId}</Text>}
    {event.issuedBy && <Text style={styles.meta}>By: {event.issuedBy}</Text>}
  </Pressable>;
});

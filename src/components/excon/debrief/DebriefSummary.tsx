...
export function DebriefSummary({ report }: { report: DebriefReport }) {
  ...
  return <View style={styles.card}><View style={styles.heading}><Text style={styles.title}>Exercise Summary</Text>{legacyClock && <Text style={styles.legacy}>Legacy Exercise Clock</Text>}</View><View style={styles.grid}>
  ...
  </View>{report.protocolProvenance && <View style={styles.protocol}><Text style={styles.label}>Protocol</Text><Text style={styles.value}>{report.protocolProvenance.name} · {report.protocolProvenance.protocolId}@{report.protocolProvenance.version}</Text><Text style={styles.source}>{report.protocolProvenance.status} · {report.protocolProvenance.authority}</Text><Text style={styles.hash}>{report.protocolProvenance.protocolHash}</Text></View>}<Text style={styles.source}>Duration is the canonical Exercise Snapshot value; Debrief does not calculate or normalize it.</Text><Text style={styles.hash}>Replay source · {report.generatedFromReplayHash.slice(0, 16)}…</Text></View>;
}

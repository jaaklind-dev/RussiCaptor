...
export function TimelinePlaybackControls({ cursor, durationSec, previous, next, onToggle, onSeek }: { cursor: PlaybackCursor; durationSec: number; previous?: ExerciseTimelineEvent; next?: ExerciseTimelineEvent; onToggle: () => void; onSeek: (seconds: number, event?: ExerciseTimelineEvent) => void }) {
  return <View style={styles.card}><View style={styles.row}><Text style={styles.title}>Ajajoone taasesitus</Text><Text style={styles.time}>T+{cursor.simulationTimeSec}s / {durationSec}s</Text></View>
    <View style={styles.controls}>
      <Pressable style={styles.secondary} disabled={!previous} onPress={() => previous && onSeek(previous.simulationTimeSec, previous)}><Text style={styles.secondaryText}>Eelmine sündmus</Text></Pressable>
      <Pressable style={styles.primary} onPress={onToggle}><Text style={styles.primaryText}>{cursor.playing ? "Peata" : "Esita"}</Text></Pressable>
      <Pressable style={styles.secondary} disabled={!next} onPress={() => next && onSeek(next.simulationTimeSec, next)}><Text style={styles.secondaryText}>Järgmine sündmus</Text></Pressable>
    </View>
  </View>;
}

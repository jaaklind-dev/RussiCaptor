...
import { timelineSectionLabel } from "@/localization/dataDrivenEt";
...
export default function ExerciseTimelineScreen() {
  ...
  return <FlatList data={rows} keyExtractor={item => item.key} contentContainerStyle={styles.container}
    ListHeaderComponent={<View><View style={styles.top}><View><Text style={styles.title}>Õppuse ajajoon</Text><Text style={styles.subtitle}>Kirjutuskaitstud kanooniline sündmuste register</Text></View><Pressable onPress={() => router.back()}><Text style={styles.back}>Tagasi</Text></Pressable></View><ExerciseTimelineFilterBar categories={categories} severities={severities} patientId={patientId} caseManager={caseManager} search={search} grouping={grouping} onCategories={setCategories} onSeverities={setSeverities} onPatientId={setPatientId} onCaseManager={setCaseManager} onSearch={setSearch} onGrouping={setGrouping} /><Text style={styles.count}>{rows.filter(item => item.kind === "event").length} sündmust</Text></View>}
    renderItem={({ item }) => item.kind === "header" ? <Text style={styles.section}>{timelineSectionLabel(grouping, item.key.replace("HEADER:", ""))}</Text> : <ExerciseTimelineEventCard event={item.event} onPress={open} />}
    ListEmptyComponent={<Text style={styles.empty}>Filtritele vastavaid ajajoone sündmusi ei leitud.</Text>} />;
}

...
const frozen = <T>(value: T): T => { if (value && typeof value === "object") { Object.values(value as Record<string, unknown>).forEach(frozen); if (!Object.isFrozen(value)) Object.freeze(value); } return value; };
...
export function generateAnalytics(debrief: DebriefReport, registry: AnalyticsProviderRegistry, configuration: AnalyticsConfiguration, protocolAssessment?: ProtocolAssessmentReport): AnalyticsReport {
  ...
  const context: AnalyticsEvaluationContext = frozen({ debrief, exerciseId: debrief.exerciseId, sourceDebriefHash, configuration: frozen(structuredClone(configuration)), ...(protocolAssessment ? { protocolAssessment } : {}) });

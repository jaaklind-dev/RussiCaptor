...
export type MtpAction = "MTP_ACTIVATION" | "MTP_SET_VASCULAR_ACCESS_COUNT" | "RBC_ADMINISTRATION" | "PLASMA_ADMINISTRATION" | "PLATELET_ADMINISTRATION" | "CALCIUM_ADMINISTRATION";
...
const labels: Record<MtpAction, readonly [string, string]> = {
  ...
  MTP_SET_VASCULAR_ACCESS_COUNT: ["Veeniteede arv muudeti", "Veeniteede arv muudeti"],
...
export function handleMtpCommand(command: Readonly<{ commandId: string; exerciseId: string; patientId: string; action: MtpAction; units?: number; issuedBy: string;
  deliveryMode?: "GRAVITY" | "PRESSURE_BAG" | "RAPID_INFUSER"; vascularAccessLineId?: "IV-1" | "IV-2" | "IV-3"; vascularAccessCount?: number }>): MtpCommandResult {
  ...
  const applied = getInstructorRuntimeOwner(command.exerciseId, command.patientId)?.executeMtpAction?.(command.commandId, command.action, command.units ?? 1,
    { deliveryMode: command.deliveryMode, vascularAccessLineId: command.vascularAccessLineId, count: command.vascularAccessCount });
  ...
  if (result.ok) { const simulationTimeSec = getCanonicalPatientRuntimeSnapshot(command.patientId)?.state.exerciseTimeSec ?? 0; const [title, description] = labels[command.action];
    addTimelineEvent({ id: `TL-MTP-${command.commandId}`, exerciseId: command.exerciseId, patientId: command.patientId, timestamp: `T+${simulationTimeSec}s`, simulationTimeSec,
      type: "intervention", title, description: `${description}${command.action === "MTP_ACTIVATION" || command.action === "CALCIUM_ADMINISTRATION" || command.action === "MTP_SET_VASCULAR_ACCESS_COUNT" ? "" :

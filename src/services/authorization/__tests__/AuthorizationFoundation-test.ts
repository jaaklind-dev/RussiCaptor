...
describe("WP-41A authorization foundation", () => {
  ...
  it("resolves EXCON permissions canonically, recursively immutable and independent of assignment order", () => {
    ...
    expect(first).toEqual(["EXCON_EXERCISE_CONTROL", "EXERCISE_JOIN", "EXERCISE_RUNTIME_RECOVERY", "INSTRUCTOR_EVALUATION_READ", "INSTRUCTOR_EVALUATION_WRITE"]); expect(first).toEqual(second);

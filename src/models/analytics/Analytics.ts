...
export type AnalyticsCategory = "EXERCISE_FLOW" | "PATIENT_CARE" | "OWNERSHIP" | "INTERVENTIONS" | "MEDICATIONS" | "DIAGNOSTICS" | "RESOURCES" | "COMMANDS" | "ASSESSMENT" | "SYSTEM";
...
export type MetricEvidenceReference = Readonly<{ sourceType: "TIMELINE_EVENT" | "PATIENT_SUMMARY" | "EXERCISE_SUMMARY" | "AUDIT_EVENT" | "DEBRIEF_FIELD" | "ASSESSMENT_RESULT" | "ASSESSMENT_REPORT"; sourceId?: string; patientId?: string; simulationTimeSec?: number; fieldPath?: string }>;
export type AssessmentMetricProvenance = Readonly<{ protocolId: string; protocolVersion: string; protocolHash: string; assessmentVersion: number; assessmentHash: string }>;
type MetricResultBase = Readonly<{ metricId: string; metricVersion: string; providerId: string; scope: MetricScope; subjectId?: string; category: AnalyticsCategory; evidence: readonly MetricEvidenceReference[]; assessmentProvenance?: AssessmentMetricProvenance }>;
...
export type AnalyticsEvaluationContext = Readonly<{ debrief: DebriefReport; exerciseId: string; sourceDebriefHash: string; configuration: AnalyticsConfiguration; protocolAssessment?: ProtocolAssessmentReport }>;
export interface AnalyticsMetricProvider { readonly providerId: string; readonly version: string; readonly allowEmptyScopes?: readonly MetricScope[]; getDefinitions(): readonly MetricDefinition[]; evaluate(context: AnalyticsEvaluationContext): readonly MetricResult[]; }
